...
export const init = (data) => {
    ...
    if (api_type === 'assistant') {
        chatData = localStorage.getItem("block_ollama_chat_data")
        if (chatData) {
            chatData = JSON.parse(chatData)
            if (chatData[blockId] && chatData[blockId]['threadId'] && persistConvo === "1") {
                fetch(`${M.cfg.wwwroot}/blocks/ollama_chat/api/thread.php?thread_id=${chatData[blockId]['threadId']}`)
                ...
                .catch(error => {
                    chatData[blockId] = {}
                    localStorage.setItem("block_ollama_chat_data", JSON.stringify(chatData));
                })
    ...
    document.querySelector(`.block_ollama_chat[data-instance-id='${blockId}'] #openai_input`).addEventListener('keyup', e => {
    ...
    document.querySelector(`.block_ollama_chat[data-instance-id='${blockId}'] #go`).addEventListener('click', e => {
    ...
    document.querySelector(`.block_ollama_chat[data-instance-id='${blockId}'] #refresh`).addEventListener('click', e => {
        clearHistory(blockId)
    })

    document.querySelector(`.block_ollama_chat[data-instance-id='${blockId}'] #popout`).addEventListener('click', e => {
        if (document.querySelector('.drawer.drawer-right')) {
            document.querySelector('.drawer.drawer-right').style.zIndex = '1041'
        }
        document.querySelector(`.block_ollama_chat[data-instance-id='${blockId}']`).classList.toggle('expanded')
    })
...
const addToChatLog = (type, message, blockId) => {
    let messageContainer = document.querySelector(`.block_ollama_chat[data-instance-id='${blockId}'] #ollama_chat_log`)
    ...
    messageContainer.closest('.block_ollama_chat > div').scrollTop = messageContainer.scrollHeight
}
...
const clearHistory = (blockId) => {
    ...
    document.querySelector(`.block_ollama_chat[data-instance-id='${blockId}'] #ollama_chat_log`).innerHTML = ""
}
...
const createCompletion = (message, blockId, api_type) => {
    ...
    document.querySelector(`.block_ollama_chat[data-instance-id='${blockId}'] #control_bar`).classList.add('disabled')
    document.querySelector(`.block_ollama_chat[data-instance-id='${blockId}'] #openai_input`).classList.remove('error')
    document.querySelector(`.block_ollama_chat[data-instance-id='${blockId}'] #openai_input`).placeholder = questionString
    document.querySelector(`.block_ollama_chat[data-instance-id='${blockId}'] #openai_input`).blur()
    addToChatLog('bot loading', '...', blockId);

    fetch(`${M.cfg.wwwroot}/blocks/ollama_chat/api/completion.php`, {
        method: 'POST',
        body: JSON.stringify({
            message: message,
            history: history,
    ...
    .then(response => {
        ...
        document.querySelector(`.block_ollama_chat[data-instance-id='${blockId}'] #control_bar`).classList.remove('disabled')
    ...
    .then(data => {
        ...
        document.querySelector(`.block_ollama_chat[data-instance-id='${blockId}'] #openai_input`).focus()
    })
    .catch(error => {
        console.log(error)
        document.querySelector(`.block_ollama_chat[data-instance-id='${blockId}'] #openai_input`).classList.add('error')
        document.querySelector(`.block_ollama_chat[data-instance-id='${blockId}'] #openai_input`).placeholder = errorString
    })
}
...
const buildTranscript = (blockId) => {
    let transcript = []
    document.querySelectorAll(`.block_ollama_chat[data-instance-id='${blockId}'] .openai_message`).forEach((message, index) => {
        if (index === document.querySelectorAll(`.block_ollama_chat[data-instance-id='${blockId}'] .openai_message`).length - 1) {
            return
        }
