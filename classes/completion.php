...
class completion {
    ...
    public function __construct($model, $message, $history, $block_settings) {
        // Set default values
        $this->model = $model;
        $this->apikey = get_config('block_ollama_chat', 'apikey');
        ...
        $this->prompt = $this->get_setting('prompt', get_string('defaultprompt', 'block_ollama_chat'));
        $this->assistantname = $this->get_setting('assistantname', get_string('defaultassistantname', 'block_ollama_chat'));
        $this->username = $this->get_setting('username', get_string('defaultusername', 'block_ollama_chat'));
    ...
    protected function get_setting($settingname, $default_value = null) {
        $setting = get_config('block_ollama_chat', $settingname);
    ...
    private function build_source_of_truth($localsourceoftruth) {
        $sourceoftruth = get_config('block_ollama_chat', 'sourceoftruth');
    
        if ($sourceoftruth || $localsourceoftruth) {
            $sourceoftruth = 
                get_string('sourceoftruthpreamble', 'block_ollama_chat')
                . $sourceoftruth . "\n\n"
                . $localsourceoftruth . "\n\n";
            }
