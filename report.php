...
$pageurl = $CFG->wwwroot . "/blocks/ollama_chat/report.php?courseid=$courseid" .
    "&user=$user" .
    "&starttime=$starttime" .
    "&endtime=$endtime";
...
require_capability('block/ollama_chat:viewreport', $context);

$datetime = new DateTime();
$table = new \block_ollama_chat\report(time());
$table->show_download_buttons_at(array(TABLE_P_BOTTOM));
$table->is_downloading(
    $download, 
    get_string('downloadfilename', 'block_ollama_chat') 
        . '_' 
...
if (!$table->is_downloading()) {
    ...
    $PAGE->set_title(get_string('ollama_chat_logs', 'block_ollama_chat'));
    $PAGE->set_heading(get_string('ollama_chat_logs', 'block_ollama_chat'));
    $PAGE->navbar->add($course->shortname, new moodle_url('/course/view.php', ['id' => $course->id]));
    $PAGE->navbar->add(get_string('ollama_chat_logs', 'block_ollama_chat'), new moodle_url($pageurl));

    echo $OUTPUT->header();
    echo $OUTPUT->render_from_template('block_ollama_chat/report_page', [
        "courseid" => $courseid,
        "user" => $user,
        "starttime" => $starttime,
        "endtime" => $endtime,
        "link" => (new moodle_url("/blocks/ollama_chat/report.php"))->out()
    ]);
}
